...
export const register = form(credentialsSchema, async ({ email, password }): Promise<AuthResult> => {
	...
	const loginResult = await createAuthClient().auth.login('customer', 'emailpass', {
		email,
		password
	})
...
export const requestResetPassword = form(v.object({ email: v.pipe(v.string(), v.nonEmpty(), v.email()) }), async ({ email }): Promise<AuthResult> => {
	try {
