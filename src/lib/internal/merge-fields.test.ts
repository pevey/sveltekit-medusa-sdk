...
describe('mergeFields', () => {
	...
	it('appends a consumer addition while keeping the baseline', () => {
		expect(mergeFields(BASE, '+variants.inventory_quantity')).toBe('*variants.calculated_price,+variants.inventory_quantity')
	})
	...
	it('handles multiple baseline tokens independently', () => {
		...
		expect(mergeFields(base, '+variants.inventory_quantity')).toBe('*variants.calculated_price,+categories,+variants.inventory_quantity')
	})
})
